"""Trusted operator CLI for capability #18 Blind Analysis attestation.

The execution receipt must be produced by
``build_blind_analysis_execution_receipt`` in an external process configured
with the real blinded runners.  This CLI only validates and anchors it.  The
HMAC key is read from ``INFINITY_MATURITY_HMAC_KEY_B64`` and must never be
exposed to ordinary pull-request code.
"""
...
from research_engine.blind_analysis_attestor import attest_blind_analysis_proofs
...
def _read_anchor(path: str | None) -> str:
    ...
    info = candidate.stat()
    if not candidate.is_file() or not 1 <= info.st_size <= _MAX_ANCHOR_BYTES:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate a trusted blind-analysis execution receipt and mint only "
            "capability #18 execution/independent/reproducibility proofs."
        )
    )
    ...
    try:
        ...
        inventory = {
            ...
            "note": (
                "Blindness/independent structure is process evidence; it does not "
                "prove the underlying scientific answer true."
            ),
        }
        ...
        print(
            f"BLIND ANALYSIS ATTESTATION: FAIL ({type(exc).__name__}: {exc})",
            file=sys.stderr,
        )
        return 1

    print("BLIND ANALYSIS ATTESTATION: PASS")
    ...
    print(
        "NOTE: blindness and repeatability are process evidence, not proof of truth."
    )
    return 0
