"""Trusted external-validator CLI for #36/#37 independence evidence.

The execution receipt must already have been produced by at least three distinct
external validator domains.  This command validates that receipt against the
exact clean repository revision and mints only ``independent_validation`` for
Red-Team AI (#36) and Devil's Advocate Swarm (#37).

The HMAC key is read only from ``INFINITY_MATURITY_HMAC_KEY_B64``.  Ledger,
anchor and inventory outputs must live outside the repository.  PASS validates
the independence process; it does not prove any challenged scientific claim.
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate an external #36/#37 independence receipt and mint only "
    ...
    try:
        ...
        capability_36 = result.audit.maturity_report.results[35]
        capability_37 = result.audit.maturity_report.results[36]
        inventory = {
            "schema_version": 1,
            ...
            "validator_manifest_hash": result.validator_manifest_hash,
            ...
            "capability_36": {
                "status": capability_36.status,
                "missing_proofs": [item.value for item in capability_36.missing_proofs],
            },
            "capability_37": {
                "status": capability_37.status,
                "missing_proofs": [item.value for item in capability_37.missing_proofs],
            },
        ...
        print(
            f"ADVERSARIAL INDEPENDENCE ATTESTATION: FAIL ({type(exc).__name__}: {exc})",
            file=sys.stderr,
        )
        return 1

    print("ADVERSARIAL INDEPENDENCE ATTESTATION: PASS")
    print(f"Revision: {result.revision}")
    print(
        "#36/#37 independent receipts: "
        f"added={result.receipts_added}, reused={result.receipts_reused}"
    )
    print(
        "NOTE: external validator diversity validates process independence; "
        "agreement/disagreement does not prove a scientific claim true."
    )
    return 0
