"""Trusted external-validator CLI for capability #19 debate independence.

This command validates a pre-built external judge receipt and mints only
``independent_validation`` for capability #19.  The HMAC key is read only from
``INFINITY_MATURITY_HMAC_KEY_B64`` and outputs must live outside the repository.

A PASS means the external debate-validation process satisfied the committed
independence contract.  It does not prove that the tournament winner is true.
"""
...
from research_engine.debate_independence_attestor import attest_debate_independence
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate an external Debate Tournament independence receipt and mint "
            "only capability #19 independent-validation evidence."
        )
    )
    ...
    try:
        ...
        capability = result.audit.maturity_report.results[18]
        inventory = {
            "schema_version": 1,
        ...
        print(
            f"DEBATE INDEPENDENCE ATTESTATION: FAIL ({type(exc).__name__}: {exc})",
            file=sys.stderr,
        )
        return 1

    print("DEBATE INDEPENDENCE ATTESTATION: PASS")
    print(f"Revision: {result.revision}")
    print(
        "#19 independent receipt: "
        f"added={result.receipts_added}, reused={result.receipts_reused}"
    )
    print(
        "NOTE: independent blinded judges validate the process; their agreement or "
        "disagreement does not prove the winning hypothesis true."
    )
    return 0
