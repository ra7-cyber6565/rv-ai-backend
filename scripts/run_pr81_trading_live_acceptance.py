...
# One fixed reviewed question only.  It is deliberately a single paragraph so
# positional bullet parsing cannot create irrelevant acceptance requirements.
# Numeric rules are explicitly required to carry provenance instead of asking
# the model for arbitrary magic thresholds.
...
def _experiment_complete(row: Mapping[str, Any]) -> tuple[bool, int]:
    ...
    if isinstance(declared_missing, list):
        missing.extend(str(item) for item in declared_missing if item in REQUIRED_EXPERIMENT_SPEC)
...
def _implementation_execution(company: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    implementation = next((row for row in workers if row.get("role") == "implementation"), None)
    ...
    executed = [
        ...
        and str(row.get("tool") or "") == "isolated_build"
    ]
    return {
        "present": True,
...
def evaluate_result(result: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    hypotheses = [row for row in (result.get("hypotheses") or []) if isinstance(row, Mapping)]
    ...
    worker_ids = {str(row.get("worker_id") or "") for row in workers if row.get("worker_id")}
    chief = company.get("chief_execution") or {}
    chief_accounting = chief.get("accounting") or {}
    handoff_roles = {str(value) for value in (company.get("handoff_worker_roles") or []) if value}
    ...
    for row in hypotheses[:3]:
        ...
        hypothesis_complete.append(
            bool(row.get("is_testable") is True and row.get("has_prediction") is True and complete)
        )
    ...
    critical_trade_gaps = sorted(CRITICAL_TRADE_POINTS & (not_met | not_measured))

    task_types = {str(value) for value in (contract.get("task_types") or [])}
    checks = [
        _safe_check("status_complete", result.get("status") == "COMPLETE",
                    str(result.get("status") or "missing")),
        _safe_check("maximum_mode_executed", str(coverage.get("mode") or "").upper() == "MAXIMUM",
                    "public result reports MAXIMUM mode"),
        _safe_check("coding_not_creative", "coding" in task_types and "creative" not in task_types,
                    "task contract classifies technical implementation without CRAFT"),
        _safe_check("six_workers_requested", contract.get("explicit_min_workers") == 6,
                    "fixed acceptance requires six specialist workers"),
        _safe_check("task_contract_complete", contract.get("assessment") != "PARTIAL"
                    and contract.get("worker_requirement_gap") is False,
                    f"known_missing={len(contract.get('known_missing_deliverables') or [])}"),
        _safe_check("threshold_provenance", threshold.get("active") is True
                    and threshold.get("passed") is True
                    and int(threshold.get("unsupported_rule_lines") or 0) == 0,
                    f"checked={int(threshold.get('checked_rule_lines') or 0)}, unsupported={int(threshold.get('unsupported_rule_lines') or 0)}"),
        _safe_check("trade_contract_ran", trade.get("asked") is True and trade.get("ran") is True,
                    f"met={int(trade.get('met_count') or 0)}/{int(trade.get('contract_points') or 0)}"),
        _safe_check("critical_trade_contract", not critical_trade_gaps,
                    f"critical_gaps={len(critical_trade_gaps)}"),
        _safe_check("no_chased_win_rate", not (trade.get("chased_win_rate") or []),
                    "no 90%+ claimed win-rate accepted"),
        _safe_check("trading_reality_boundaries", trade.get("live_tested") is False
                    and trade.get("broker_connected") is False
                    and trade.get("financial_advice") is False,
                    "no fake live/broker/advice state"),
        _safe_check("three_structured_hypotheses", len(hypotheses) >= 3
                    and len(hypothesis_complete) == 3 and all(hypothesis_complete),
                    f"hypotheses={len(hypotheses)}, first3_missing_spec_fields={hypothesis_missing_counts}"),
        _safe_check("six_specialists_executed", len(workers) == len(ready_workers) == 6
                    and roles == EXPECTED_ROLES and len(worker_ids) == 6
                    and all(_positive_int((row.get("accounting") or {}).get("actual_http_attempts"))
                            and _positive_int((row.get("accounting") or {}).get("successful_calls"))
                            for row in workers),
                    f"ready={len(ready_workers)}/6, roles={len(roles)}/6"),
        _safe_check("specialist_handoff_complete", company.get("handoff_prepared") is True
                    ...
                    f"handoff_roles={len(handoff_roles)}/6, truncated={len(company.get('handoff_truncated_roles') or [])}"),
        _safe_check("company_accounting_complete", company.get("accounting_complete") is True
                    and all(row.get("accounting_complete") is True for row in workers),
                    "worker usage receipts are complete"),
        _safe_check("chief_executed", {"analysis", "synthesis"}.issubset(set(chief.get("done_passes") or []))
                    and _positive_int(chief_accounting.get("successful_calls")),
                    "chief analysis+synthesis have execution receipts"),
        _safe_check("implementation_build_executed", implementation["executed"] is True,
                    f"isolated_build_receipts={implementation['executed_receipts']}"),
        _safe_check("no_false_replication", company.get("independent_scientific_replication") is False
    ...
    return {
        ...
        "summary": {
            ...
            "known_missing_deliverables": len(contract.get("known_missing_deliverables") or []),
            "threshold_rule_lines_checked": int(threshold.get("checked_rule_lines") or 0),
            "threshold_rule_lines_unsupported": int(threshold.get("unsupported_rule_lines") or 0),
            ...
            "handoff_compacted_roles": len(company.get("handoff_compacted_roles") or []),
            "handoff_truncated_roles": len(company.get("handoff_truncated_roles") or []),
...
def execute(receipt_path: Path | None = None) -> Dict[str, Any]:
    ...
    try:
        ...
        public.update(
            ...
            checks=[_safe_check("trading_max_live_execution", False,
