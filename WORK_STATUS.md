...
|---|---|---|---|
| Windows live ₹0 gate + launcher hardening | ChatGPT | CODE COMPLETE / CI PENDING | PowerShell runner accepts only non-secret data-root/receipt arguments; preflight now probes an absolute writable out-of-repo root and minimum free space before any provider call. Live crashes produce sanitized receipts, and backend startup is repo-relative instead of hard-coded to one `C:\Users\...` checkout. |
