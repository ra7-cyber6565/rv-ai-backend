...
def test_explicit_code_test_and_external_routes_compile_without_evidence(tmp_path):
    base = _base()
    ...
    # A policy compiler has no ledger/evidence field and cannot make a claim verified.
...
def test_symlink_manifest_fails_closed(tmp_path):
    ...
    target.write_text('{"schema_version":1,"code_test_bindings":[],"external_routes":[]}', encoding="utf-8")
