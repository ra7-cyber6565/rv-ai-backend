...
def test_explicit_parts_use_measured_semantic_ledger_not_positional_ids():
    ...
    report = assess_contract(
        contract,
        _result(hypotheses=True, math_model=True, falsification=True),
    )
    ...
    assert rows["part_3"]["output_reference"] == "contract_ledger:falsification"
...
def test_one_failed_measurement_keeps_corresponding_explicit_part_partial():
    ...
    report = assess_contract(
        contract,
        _result(hypotheses=True, math_model=False, falsification=True),
    )
    rows = _coverage(report)
...
def test_multi_demand_part_requires_every_measured_demand():
    ...
    report = assess_contract(
        contract,
        _result(math_model=True, falsification=False),
    )
    row = _coverage(report)["part_1"]

    assert row["measured_keys"] == ["math_model", "falsification"]
