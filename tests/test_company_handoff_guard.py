...
from research_engine import company_handoff_guard as guard
...
def rich_report():
    ...
    for index in range(6):
        plan = {field: f"{field} explicit proposed-plan wording" for field in PLAN_FIELDS}
        plan["variables"] = [
            {
                "symbol": f"x{j}",
                "definition": "explicit variable definition",
...
def test_rich_structured_reports_use_bounded_handoff_without_false_10_of_11_gap():
    raw = rich_report()
    # Worker capture is 24k. The old fixture exceeded it before JSON parsing,
    # so it never exercised the 16k chief handoff at all.
    assert 16000 < len(raw) < 24000
    ...
    assert set(result["handoff_compaction_levels"].values()) == {"LOSSLESS"}
    assert "LOSSLESS_SHARED_HYPOTHESIS_FIELDS_V1" in handoff
    ...
    assert any("lossless shared fields" in note for note in passes["notes"])
...
def test_lossless_handoff_preserves_full_falsification_variables_and_opposing_evidence():
    result = company.run_company("Q", packet(), get_depth_config("COMPANY"),
                                 worker=lambda payload: envelope(rich_report()))
    before = copy.deepcopy(result["workers"])
    text = company.chief_handoff(result)
    decoded = [json.loads(line.split("> ", 1)[1]) for line in text.splitlines()
               if line.startswith("DATA> ")]
    assert len(decoded) == 4
    for sent, original in zip(decoded, before):
        view = sent["report"]
        restored = copy.deepcopy(view["report"])
        restored["hypotheses"] = [dict(view["shared_hypothesis_fields"], **row)
                                   for row in restored["hypotheses"]]
        assert restored == original["report"]
    assert result["workers"] == before


def test_distinct_long_plans_are_not_summarized_into_complete_handoff():
    report = company.normalize_report(rich_report(), ["S1"])
    for i, hypothesis in enumerate(report["hypotheses"]):
        hypothesis["test_plan"] = {
            field: (f"hypothesis {i} {field} " * 12) + "ONLY REJECT AFTER EXTERNAL REPLICATION"
            for field in PLAN_FIELDS
        }
    result = {"workers": [{"role": role, "status": "DRAFT_READY", "report": report}
                           for role, _ in company.ROLES[:4]]}
    text = company.chief_handoff(result)
    assert len(result["handoff_truncated_roles"]) == 4
    assert all(role in text for role, _ in company.ROLES[:4])
    assert all(reason == "full_report_exceeds_handoff_budget"
               for reason in result["handoff_compaction_blocked_reasons"].values())


def test_worker_capture_limit_is_not_bypassed_to_make_rich_fixture_pass():
    raw = rich_report()
    huge = json.loads(raw)
    huge["summary"] = "oversized worker output " * 1500
    result = company.run_company("Q", packet(), get_depth_config("COMPANY"),
                                 worker=lambda payload: envelope(json.dumps(huge)))
