"""Release-contract regressions for the evidence-before-generation boundary.

These tests close a runtime fail-open gap: P0-B can be perfectly implemented in
orchestrator.py, but the independent final gate must still reject a production
research result when the evidence-first audit is missing or internally says
achievement was not reached. Legacy standalone callers remain opt-in compatible.
"""
...
def test_production_request_contract_requires_evidence_first_audit():
    ...
    parsed = FQ.QualityContract.from_mapping(raw)
    assert parsed.evidence_first_required is True


def test_legacy_standalone_contract_remains_explicitly_opt_in():
    # Backward compatibility is deliberate, not an accidental missing-field
    # success path. Production requested.quality_contract() opts in above.
...
def test_mutated_false_evidence_first_achievement_blocks_final_gate():
    spec = replace(FQ.QualityContract(), evidence_first_required=True)
    ctx = _base_context()
    ctx.update({
...
def test_complete_nonvacuous_evidence_first_audit_passes_new_contract_checks():
    spec = replace(FQ.QualityContract(), evidence_first_required=True)
    ctx = _base_context()
    ctx.update({
        "evidence_first_required": True,
