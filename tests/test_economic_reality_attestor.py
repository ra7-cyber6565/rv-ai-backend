...
import research_engine.economic_reality_attestor as attestor_mod
from research_engine.capability_registry import ProofKind
from research_engine.economic_reality_attestor import (
    attest_economic_reality_execution,
    run_economic_reality_benchmark,
)
...
def test_analytic_benchmark_passes_and_preserves_truth_boundary():
    result = run_economic_reality_benchmark()
...
def test_trusted_attestor_mints_only_execution_and_reproducibility(tmp_path):
    ledger_path = tmp_path / "economic-proof.jsonl"
    result = attest_economic_reality_execution(
        repo_root=_root(),
        ledger_path=ledger_path,
        integrity_key=KEY,
        run_reference="economic-reality:ci-fixture-1",
        now=NOW,
    )
    ...
    assert result.real_world_viability_proven is False
    assert result.truth_proven is False

    capability = result.audit.maturity_report.results[99]
    ...
    assert {row["subject"] for row in rows} == {"economic-reality-benchmark"}


def test_existing_ledger_requires_anchor_and_same_run_is_idempotent(tmp_path):
    ledger_path = tmp_path / "economic-proof.jsonl"
    first = attest_economic_reality_execution(
        repo_root=_root(),
        ledger_path=ledger_path,
        integrity_key=KEY,
        run_reference="economic-reality:ci-fixture-2",
        now=NOW,
    )
    ...
    second = attest_economic_reality_execution(
        repo_root=_root(),
        ledger_path=ledger_path,
        integrity_key=KEY,
        run_reference="economic-reality:ci-fixture-2",
...
def test_ledger_inside_repo_is_rejected(tmp_path):
    target = _root() / ".economic-proof-test.jsonl"
    try:
        with pytest.raises(ValueError, match="outside"):
            attest_economic_reality_execution(
                ...
                ledger_path=target,
                integrity_key=KEY,
                run_reference="economic-reality:inside-repo",
                now=NOW,
            )
    finally:
...
def test_failed_benchmark_cannot_mint_receipts(monkeypatch, tmp_path):
    original = attestor_mod.run_economic_reality_benchmark
    ...
    monkeypatch.setattr(attestor_mod, "run_economic_reality_benchmark", failed)
    ledger_path = tmp_path / "economic-proof.jsonl"
    with pytest.raises(ValueError, match="benchmark failed"):
        attest_economic_reality_execution(
            ...
            ledger_path=ledger_path,
            integrity_key=KEY,
            run_reference="economic-reality:failed",
            now=NOW,
        )
    assert not ledger_path.exists()
...
def test_nondeterministic_repeated_execution_cannot_mint_reproducibility(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(attestor_mod, "run_economic_reality_benchmark", changing)
    ledger_path = tmp_path / "economic-proof.jsonl"
    with pytest.raises(ValueError, match="not deterministic"):
        attest_economic_reality_execution(
            ...
            ledger_path=ledger_path,
            integrity_key=KEY,
            run_reference="economic-reality:nondeterministic",
            now=NOW,
        )
    assert not ledger_path.exists()
