...
def test_relevant_official_snippet_can_cover_provenance_lane_without_truth_upgrade():
    ...
    # Lane qualification says only that a usable official-record source exists.
    # It must not invent a truth/evidence confidence field.
...
def test_primary_traditional_text_requires_actual_text_access_but_not_empirical_truth():
    ...
    assert receipt["qualified"] is True
    # A traditional text documents the teaching; empirical proposition support
    # is intentionally not required and is not inferred.
...
def test_installed_report_uses_qualified_coverage_not_candidate_count():
    # research_engine.__init__ installs the wrapper before this import resolves.
