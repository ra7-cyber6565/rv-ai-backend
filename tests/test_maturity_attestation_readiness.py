...
def test_current_specialized_attestors_are_explicit_and_external_boundaries_remain_visible():
    ...
    debate_tournament = _route(report, 19, ProofKind.INDEPENDENT)
    assert debate_tournament.status == "SPECIALIZED_EXTERNAL_ATTESTOR"
    assert debate_tournament.attestor_id == "debate-tournament-independent"
    assert debate_tournament.external_required is True
