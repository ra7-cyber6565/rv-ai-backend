...
Additive/fail-safe validation layer: existing research output is preserved,
plans never become results, and missing values remain explicit.
"""
from __future__ import annotations
from copy import deepcopy
from typing import Any, Dict, Mapping, Optional, Sequence
from .validation_contracts import (
    AGENT_ID, CONDITIONAL_PASS, EXPERIMENT_FIELDS, FAIL, HYPOTHESIS_STATUSES, INCONCLUSIVE,
    NOT_TESTED, PASS, REQUIRED_SECTIONS, RESULT_OBSERVED, SCHEMA_VERSION, TEST_PERFORMED,
    TEST_POSSIBLE, TEST_PROPOSED, TEST_STATES, TO_BE_ESTIMATED, UNKNOWN,
    existing_experiment_intelligence, extract_hypotheses, is_trading, meaningful,
    second_pass_summary, source_count, text,
)
...
def _blockers(experiments: Sequence[Mapping[str, Any]], confidence: Mapping[str, Any], trading: bool) -> list:
    blockers = []
    if not experiments: blockers.append("No structured domain hypothesis supplied to AI-2.")
    for e in experiments:
        blockers += [f"{e.get('hypothesis_id', 'hypothesis')}: missing {field}." for field in e.get("missing_required_fields", [])]
...
def _cannot_test(experiments: Sequence[Mapping[str, Any]]) -> list:
    rows = []
    if not experiments: rows.append("Domain hypotheses cannot be validated because no structured hypothesis reached AI-2.")
    if not any(e.get("Dataset/sample") != UNKNOWN for e in experiments): rows.append("Effect size/CI/power/bootstrap/permutation/replication require a concrete dataset/sample.")
    if not any(e.get("test_state") in {TEST_PERFORMED, RESULT_OBSERVED} for e in experiments): rows.append("No claim can be promoted to PASS/FAIL from a proposed protocol alone.")
    if not any(e.get("test_state") == RESULT_OBSERVED for e in experiments): rows.append("Observed performance/effect cannot be asserted without result provenance and recorded outputs.")
...
def validate_ai2_packet(packet: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    missing = [name for name in REQUIRED_SECTIONS if name not in sections]; errors = []
    if missing: errors.append("missing_required_sections")
    ...
    if not isinstance(rows, Sequence): errors.append("experiment_rows_not_sequence"); rows = []
    for row in rows:
        if not isinstance(row, Mapping): errors.append("experiment_row_not_mapping"); continue
...
class AI2ValidationDirector:
    def build_packet(self, question: str, research_result: Optional[Mapping[str, Any]],
        ...
        experiment_section = {
            "domain_hypothesis_experiments": experiments, "required_contract_fields": list(EXPERIMENT_FIELDS),
        ...
        if trading: experiment_section["trading_validation_standard"] = trading_standard(hypotheses, result)
        sections: Dict[str, Any] = {
            "1. Interpretation of User Goal": {"original_question": text(question, ""),
                "goal": "Determine whether the proposed explanation/model/strategy is true, false, useful, robust or only an attractive story using discriminating observations.",
                "deliverable": "Auditable quantitative validation plan plus honest status of what is actually tested.",
                "do_not_change_task_rule": "Measurement convenience must not silently redefine the requested outcome.",
                "measurement_limits": "Separate directly measurable outcomes, defensible proxies, unobservable constructs and currently unavailable measurements."},
            "2. Quantifiable Components": {"variables_by_hypothesis": [{"hypothesis_id": e["hypothesis_id"], "variables": e["Variables"]} for e in experiments],
                ...
                "measurement_rule": "Do not replace target with easier proxy without separate proxy-validity evidence.",
        ...
        packet = {"title": "AI-2 VALIDATION PACKET", "agent_id": AGENT_ID, "schema_version": SCHEMA_VERSION,
        ...
        if second_pass_outputs: packet["second_pass_context"]["agent_outputs"] = deepcopy(dict(second_pass_outputs))
        packet["packet_integrity"] = validate_ai2_packet(packet)
        return packet


def attach_ai2_validation(question: str, research_result: Mapping[str, Any], director: Optional[AI2ValidationDirector] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        enriched["ai2_validation"] = {"title": "AI-2 VALIDATION PACKET", "agent_id": AGENT_ID,
            ...
            "truth_policy": {"never_invent_results": True}, "packet_integrity": {"valid": False, "errors": ["packet_build_failed"]}}
    return enriched
