...
This module is a deterministic, zero-call post-research governance layer.  It
turns the engine's already-measured research artifacts into the exact AI-1
handoff packet used by the four-agent research company.

It deliberately does NOT manufacture missing research.  Missing source text,
claim support, mechanisms, data, experiments, or contradiction work are emitted
as explicit missing states and ranked second-pass tasks.

Design rules:
- retrieved != read != cited != supporting
- source count != research quality
- C/D/E evidence grades never become facts silently
- no guessed experts, source access, statistics, experiments, or backtests
- all confidence numbers here describe packet/process completeness, never truth,
...
SCHEMA_VERSION = "ai1-research-packet-1.0"
...
_DOMAIN_RESEARCH_TARGETS: Mapping[str, Tuple[str, ...]] = {
    "scientific": (
        "experimental science", "causal inference", "replication research",
        "systematic review / meta-analysis",
    ),
    "medical": (
        "clinical epidemiology", "evidence-based medicine", "causal inference",
        "systematic review / meta-analysis",
    ),
    "mathematical": (
        "mathematical modeling", "statistics", "optimization", "uncertainty quantification",
    ),
    "technical": (
        "computer science", "software engineering", "systems engineering", "benchmarking",
    ),
    "financial": (
        "finance", "econometrics", "market microstructure", "behavioral finance",
    ),
    "psychological": (
        "cognitive science", "behavioral science", "neuroscience", "replication research",
    ),
    "historical": (
        "primary-source history", "historiography", "archival research", "source criticism",
    ),
    "philosophical": (
        "analytic philosophy", "history of ideas", "conceptual analysis",
    ),
}
...
def _source_pool(result: Mapping) -> List[Dict]:
    """Collect every source surface the public result actually exposes."""
    merged: List[Dict] = []
    seen = set()
...
def _access_depth(source: Mapping) -> str:
    ...
    chars = int(source.get("full_text_chars") or 0)
    pages = int(source.get("pages_read") or 0)
    total = int(source.get("pages_total") or 0)
    if chars > 0 or level == "full_text":
        return "FULL TEXT ACCESSED"
    if level == "claims" or (pages > 0 and total > pages):
...
def _source_strength(source: Mapping) -> float:
    ...
    primary_bonus = 0.08 if source.get("is_primary") is True else 0.0
    peer_bonus = 0.06 if source.get("peer_reviewed") is True else 0.0
    retraction_penalty = 1.0 if source.get("retracted") is True else 0.0
    # Hierarchy is a preference, not a proof; quality/relevance dominate.
    return round((quality * 0.42) + (relevance * 0.38) + access_bonus
                 + primary_bonus + peer_bonus + ((10 - hierarchy) * 0.006)
                 - retraction_penalty, 4)
...
def _source_limitations(source: Mapping) -> List[str]:
    ...
    if relevance is not None and float(relevance or 0) < 0.25:
        limits.append("low direct relevance")
    access = _access_depth(source)
    if access in {"METADATA ONLY", "SNIPPET ONLY", "ABSTRACT ONLY"}:
...
def _strongest_sources(sources: Sequence[Mapping], limit: int = 12) -> List[Dict]:
    ranked = sorted(sources, key=lambda s: (_source_strength(s), -_source_hierarchy_rank(s)),
                    reverse=True)
    out: List[Dict] = []
    for source in ranked[:limit]:
        out.append({
            "source_id": _text(source.get("source_id"), 40),
            "title": _text(source.get("title") or MISSING_SOURCE, 240),
            "contribution": _text(
                source.get("contribution") or source.get("domain_verdict", {}).get("reason")
                if isinstance(source.get("domain_verdict"), Mapping) else "",
                300,
            ) or "Contribution must be read from the claim-evidence links; not guessed here.",
...
def _claim_rows(result: Mapping) -> List[Dict]:
    ...
    checks = _safe_dict(verification.get("claim_checks"))
...
def _source_check_passes_ae(row: Mapping) -> bool:
    for src in _safe_list(row.get("source_checks")):
        src = _safe_dict(src)
        if src.get("passes_ae") is True:
            return True
    checks = _safe_list(row.get("checks"))
    statuses = {}
    for check in checks:
...
def _grade_claim(row: Mapping) -> Tuple[str, str]:
    ...
    result = _text(row.get("result") or row.get("verdict"), 80).casefold()
    if "hypothesis" in epistemic:
        return "D", EVIDENCE_GRADES["D"]
    if "speculation" in epistemic or "unsupported" in result or "contradicted" in result:
        return "E", EVIDENCE_GRADES["E"]
    if "inference" in epistemic:
        return "C", EVIDENCE_GRADES["C"]
    if _source_check_passes_ae(row) and "supported" in result and "partial" not in result:
        return "A", EVIDENCE_GRADES["A"]
    if ("partial" in result or "source_reported" in result
            or _text(row.get("entailment_check"), 20).casefold() == "pass"):
        return "B", EVIDENCE_GRADES["B"]
    if "unable" in result or "unver" in result or not result:
        return "C", EVIDENCE_GRADES["C"]
    return "E", EVIDENCE_GRADES["E"]
...
def _claim_matrix(result: Mapping) -> List[Dict]:
    ...
    contradictions = _safe_list(result.get("contradictions"))
    matrix: List[Dict] = []
    for row in rows:
        ...
        if row.get("contradicted"):
            contra.append(_safe_dict(row.get("contradiction_span")) or {
                "status": "contradicted; exact span not exposed"
            })
        # Global contradictions are kept separate unless the runtime explicitly
        # links them; title/keyword matching here would fabricate a relationship.
        matrix.append({
            "claim_id": _text(row.get("claim_id"), 40),
            "claim": _text(row.get("text") or row.get("claim"), 1200),
            "supporting_evidence": {
            ...
            "mechanism": (_text(row.get("mechanism"), 500) or
                          ("claim itself contains an explicit mechanism cue"
                           if _MECHANISM_CUES.search(_text(row.get("text"), 1200))
                           else UNKNOWN)),
            ...
            "promotion_guard": (
                ...
                if grade in {"C", "D", "E"} else
                "A/B still require wording proportional to evidence; grade is not absolute truth"
            ),
    ...
    if not matrix:
        return [{
            ...
            "confidence_grade": "E",
            "confidence_label": EVIDENCE_GRADES["E"],
...
def _domains(result: Mapping, sources: Sequence[Mapping]) -> List[Dict]:
    names: List[str] = []
    for field in _safe_list(result.get("relevant_fields")):
        field = _text(field, 120)
        if field and field not in names:
            names.append(field)
    for qtype in _safe_list(result.get("question_types")):
        qtype = _text(qtype, 100)
        if qtype and qtype not in names:
            names.append(qtype)
...
def _experts_and_schools(question: str, result: Mapping, sources: Sequence[Mapping]) -> Dict:
    ...
    for source in sources:
        ...
        for author in _safe_list(raw_authors):
    ...
    return {
        "evidence_linked_experts": evidence_linked,
        "schools_or_research_traditions_to_test": [
            {"name": item,
             "status": "research target, not assumed correct",
...
def _required_deliverables(question: str, result: Mapping) -> Dict:
    ...
    unmet = _safe_list(ledger.get("unmet"))
    return {
...
def _contradictions(result: Mapping, matrix: Sequence[Mapping]) -> Dict:
    ...
    if counter_state is None:
        summary = _safe_dict(axes.get("summary"))
        # Do not infer a successful counter search from contradiction count.
        counter_state = summary.get("counter_search_performed")
    return {
...
def _theory_components(result: Mapping, matrix: Sequence[Mapping]) -> Tuple[List[Dict], List[Dict]]:
    ...
    for row in matrix:
        ...
        item = {
            ...
            "proposition": row.get("claim"),
            "evidence_grade": grade,
            "status": row.get("runtime_result"),
        }
        if grade in {"A", "B"}:
            retain.append(item)
        else:
            weak.append({**item,
                         "rule": "Do not treat as established; strengthen, test, or reject."})

    rejects = _safe_dict(result.get("rejects"))
    for item in _safe_list(rejects.get("rejected") or rejects.get("items")):
        weak.append({
            "proposition": _text(_safe_dict(item).get("hypothesis") or _safe_dict(item).get("text"), 1000),
            "evidence_grade": "E",
            "status": _safe_dict(item).get("reason") or "rejected by runtime ledger",
...
def _second_pass_tasks(missing: Sequence[Mapping], result: Mapping,
                       matrix: Sequence[Mapping]) -> List[Dict]:
    tasks: List[Dict] = []
    ...
    if FULL_TEXT_REQUIRED in codes:
        tasks.append(_task("Obtain and inspect full text / relevant sections",
                           "Critical conclusions should not rest on title/snippet/abstract-only reading.",
                           10, 9))
    if any(row.get("confidence_grade") in {"C", "D", "E"} for row in matrix):
        tasks.append(_task("Strengthen unresolved claim-evidence links",
                           "C/D/E claims need stronger direct support or explicit testing before promotion.",
                           10, 10))
    if any(item.get("detail", "").startswith("Counter-side") for item in missing):
    ...
    if any("axes" in item.get("detail", "") for item in missing):
    ...
    # De-duplicate by title and rank exactly by I × EIG.
    unique: Dict[str, Dict] = {}
    for item in tasks:
        old = unique.get(item["task"])
        if old is None or item["priority_score"] > old["priority_score"]:
            unique[item["task"]] = item
    return sorted(unique.values(), key=lambda x: (
        x["priority_score"], x["importance"], x["expected_information_gain"]
    ), reverse=True)
...
def _cross_agent_alerts(result: Mapping, matrix: Sequence[Mapping],
    ...
    if hypotheses or any(row.get("confidence_grade") == "D" for row in matrix):
        alerts.append({
            "agent": "AI-2",
            "alert": "Requires statistical/experimental testing.",
            "evidence": {
...
def _packet_confidence(result: Mapping, sources: Sequence[Mapping], matrix: Sequence[Mapping],
                       contradictions: Mapping, missing: Sequence[Mapping]) -> Dict:
    """Measured packet completeness score; explicitly not a truth probability."""
    score = 0
    ...
    deep = sum(1 for s in sources if _access_depth(s) in {
        "FULL TEXT ACCESSED", "RELEVANT SECTIONS REVIEWED"
    })
    reading_score = 0 if not sources else round(20 * deep / len(sources))
    components["deep_reading"] = {"score": reading_score, "max": 20,
                                  "deep_sources": deep, "visible_sources": len(sources)}
    score += reading_score

    real_rows = [row for row in matrix if row.get("claim") != NOT_VERIFIED]
    verified_rows = [row for row in real_rows if row.get("confidence_grade") in {"A", "B"}]
    claim_score = 0 if not real_rows else round(20 * len(verified_rows) / len(real_rows))
    components["claim_evidence"] = {"score": claim_score, "max": 20,
                                    "A_or_B": len(verified_rows), "claims": len(real_rows)}
    score += claim_score
    ...
    packet_score = 5  # exact 15-section packet is constructed deterministically below.
    components["packet_contract"] = {"score": packet_score, "max": 5}
    score += packet_score

    # Fail-closed caps: no source or no claim audit can never look near-perfect.
    if not sources:
        score = min(score, 25)
    if not real_rows:
        score = min(score, 55)
...
def _validate_packet(packet: Mapping) -> Dict:
    ...
    order = list(sections)
    order_ok = order == list(PACKET_SECTION_NAMES)
    matrix = _safe_list(sections.get("6. Claim-Evidence Matrix"))
    promotion_violations = [
        row.get("claim_id") for row in matrix
        if isinstance(row, Mapping)
        and row.get("confidence_grade") in {"C", "D", "E"}
        and row.get("fact_promotion_allowed") is not False
    ]
    forbidden_claims = []
    # The packet itself must never claim unavailable work happened.
    serialized = repr(packet)
    for phrase in ("all literature read", "entire internet read", "backtest completed"):
        if phrase.casefold() in serialized.casefold():
...
def build_ai1_research_packet(question: str, result: Mapping) -> Dict:
    """Build the exact AI-1 evidence handoff from a completed engine result."""
    ...
    missing = _missing_evidence(result, sources, matrix, contradiction_report)
    second_pass = _second_pass_tasks(missing, result, matrix)
    confidence = _packet_confidence(result, sources, matrix, contradiction_report, missing)
    ...
    sections[PACKET_SECTION_NAMES[4]] = _strongest_sources(sources)
    ...
    sections[PACKET_SECTION_NAMES[10]] = missing or [{"code": "NONE_MEASURED", "detail": "No missing-evidence condition was measured in exposed fields."}]
    ...
    packet: Dict = {
        ...
        "truth_policy": {
            ...
            "missing_labels": [MISSING_SOURCE, FULL_TEXT_REQUIRED, MISSING_DATA, NOT_VERIFIED, UNKNOWN],
        },
        "sections": sections,
    }
    packet["validation"] = _validate_packet(packet)
    # If the packet contract somehow fails, do not hide the defect behind a score.
...
def attach_ai1_research_packet(question: str, result: Dict) -> Dict:
    """Attach AI-1 packet fail-closed without breaking the legacy API result."""
    if not isinstance(result, dict):
        return result
    try:
        result["ai1_research_packet"] = build_ai1_research_packet(question, result)
    except Exception as exc:  # noqa: BLE001 - research answer must survive auxiliary audit
