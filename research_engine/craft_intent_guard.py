...
# Here "characters" and "conversation" name input data, not a creative output.
# Mask only these local phrases; screenplay/dialogue requests elsewhere survive.
_TECHNICAL_DATA_RE = re.compile(
    r"\b(?:count(?:ing|s)?|pars(?:e|ing)|encod(?:e|ing)|decod(?:e|ing)|"
    r"escap(?:e|ing)|remov(?:e|ing))\s+(?:the\s+|unicode\s+|special\s+)*characters?\b"
    r"|\bconversation\s+(?:logs?|history|records?|datasets?)\b",
    re.IGNORECASE,
)
...
def _technical_script_without_creative_context(question: str) -> bool:
    ...
    return bool(_TECHNICAL_SCRIPT_RE.search(text)) and not bool(
        _EXPLICIT_CREATIVE_RE.search(_TECHNICAL_DATA_RE.sub(" ", text))
    )
