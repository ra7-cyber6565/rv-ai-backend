...
A second, small tracked manifest may *narrow* an already-generated external
route to a repo-backed specialized attestor. It cannot create a new proof route,
override a base policy rule, change the capability/proof pair, or broaden a
route to multiple subjects/verifiers. This keeps specialized bindings explicit
without turning the generic route manifest into a large hand-edited policy.
...
def _apply_specialized_overrides(
    *,
    ...
    for index, item in enumerate(routes):
        ...
        row = by_key[key]
        # Only external routes have a non-empty reference prefix. This rejects a
        # future attempt to repurpose the override manifest for CODE/TEST.
        ...
        prefix = _token(
            item.get("reference_prefix"),
            field=f"specialized {index} reference_prefix",
        )
...
def merge_policy_route_extensions(
    *,
    ...
    If the optional primary extension file is absent, ``base_policy_bytes`` is
    returned unchanged for backwards compatibility. Specialized overrides never
    act without the primary extension because they are only allowed to narrow a
    route generated by that extension.
    """
