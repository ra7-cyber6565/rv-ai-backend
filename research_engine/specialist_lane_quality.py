...
def _qualify(source: Any, lane: str) -> Dict[str, Any]:
    ...
    # Production evidence packs have an explicit relevance score.  Absence of
    # that measurement is UNKNOWN, not a free pass.
    if relevance is None:
        reasons.append("RELEVANCE_NOT_MEASURED")
    elif relevance < MIN_RELEVANCE:
        reasons.append("RELEVANCE_BELOW_LANE_FLOOR")

    # Metadata can prove that a catalogue record exists, but not what the
    # requested source family actually says.  Official/allegation lanes need at
    # least a snippet; primary/traditional texts need actual text access.
    ...
    # Empirical and measured-frequency lanes claim to test something.  A source
    # that merely shares vocabulary cannot satisfy that family.
    ...
    # Scholarly interpretation is allowed to be conceptual rather than an
    # experiment, but it still needs a minimally credible source-quality score.
    # Primary/official/allegation sources are not forced through this threshold:
    # their value can be provenance/context, not scientific authority.
    ...
    # Keep reason order deterministic and bounded.
...
def qualify_lane_report(report: Dict[str, Any], plan: Mapping[str, Any] | None, pack: Any) -> Dict[str, Any]:
    ...
    ``source_ids``/``source_count`` remain the candidate view for old API/UI
    consumers.  New ``qualified_*`` fields are authoritative for completion.
    """
...
def _render_qualified_report(report: Dict[str, Any]) -> str:
    """Render existing lane report, adding only bounded weak/qualified notes."""
