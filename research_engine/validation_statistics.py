...
Plans and claimed summary metrics never become results. Calculations require
numeric observations plus explicit provenance. Decision thresholds are never
invented: PASS/FAIL requires an explicit machine-readable rule in the receipt.
"""
...
from .validation_contracts import FAIL, INCONCLUSIVE, NOT_TESTED, PASS, RESULT_OBSERVED, meaningful, text
...
def _percentile(sorted_values: Sequence[float], q: float) -> float:
    ...
    if len(sorted_values) == 1: return float(sorted_values[0])
...
def _decision(metrics: Mapping[str, Any], rule: Any) -> Dict[str, Any]:
    ...
    metric = str(rule.get("metric") or "").strip(); operator = str(rule.get("operator") or "").strip()
    ...
    try: observed = float(metrics[metric]); target = float(threshold)
    ...
    return {"status": PASS if _OPERATORS[operator](observed, target) else FAIL,
            "metric": metric, "observed": observed, "operator": operator,
            "threshold": target, "rule_source": "SUPPLIED_IN_RESULT_RECEIPT"}


def analyze_two_group_receipt(receipt: Mapping[str, Any]) -> Dict[str, Any]:
    provenance = _provenance(receipt)
    observations = receipt.get("observations") if isinstance(receipt.get("observations"), Mapping) else {}
    candidate = _numbers(observations.get("candidate")); baseline = _numbers(observations.get("baseline"))
    ...
    standardized: Any = diff / pooled_sd if pooled_sd > 0 else (0.0 if diff == 0 else NOT_TESTED)
    metrics: Dict[str, Any] = {
        "candidate_n": len(candidate), "baseline_n": len(baseline),
        "candidate_mean": c_mean, "baseline_mean": b_mean, "mean_difference": diff,
        "candidate_sd": c_sd, "baseline_sd": b_sd, "standardized_effect": standardized,
    }

    confidence_level = receipt.get("confidence_level"); ci: Any = NOT_TESTED
    if confidence_level is not None:
        try: cl = float(confidence_level)
        except (TypeError, ValueError): cl = math.nan
    ...
    bootstrap: Any = NOT_TESTED; iterations = receipt.get("bootstrap_iterations"); seed = receipt.get("random_seed")
    ...
    permutation: Any = NOT_TESTED; p_iterations = receipt.get("permutation_iterations")
    if isinstance(p_iterations, int) and p_iterations > 0 and isinstance(seed, int):
        rng = random.Random(seed); pooled = candidate + baseline; extreme = 0; observed_abs = abs(diff); n_c = len(candidate)
        for _ in range(p_iterations):
            shuffled = list(pooled); rng.shuffle(shuffled)
            if abs(mean(shuffled[:n_c]) - mean(shuffled[n_c:])) >= observed_abs: extreme += 1
    ...
    return {"observed": True, "test_state": RESULT_OBSERVED, "status": decision["status"],
            ...
            "limitations": ["No causal interpretation is implied by arithmetic alone.",
                "CI is reported only when an explicit confidence_level is supplied.",
                "Bootstrap/permutation require explicit iteration count and random_seed."]}
...
def _losing_streaks(returns: Sequence[float]) -> Tuple[int, Dict[int, int]]:
    ...
    for value in list(returns) + [0.0]:
        if value < 0: current += 1; maximum = max(maximum, current)
        elif current: histogram[current] = histogram.get(current, 0) + 1; current = 0
    return maximum, histogram


def _scenario_summary(mapping: Any) -> Any:
    if not isinstance(mapping, Mapping) or not mapping: return NOT_TESTED
    rows = []
    for name, raw in mapping.items():
        values = _numbers(raw)
        if values: rows.append({"name": str(name), "n": len(values), "expectancy": mean(values)})
    if not rows: return NOT_TESTED
    exps = [r["expectancy"] for r in rows]
    return {"scenarios": rows, "all_positive": all(x > 0 for x in exps),
            "min_expectancy": min(exps), "max_expectancy": max(exps), "expectancy_range": max(exps) - min(exps)}
...
def analyze_trading_receipt(receipt: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if not provenance or not returns:
        return {"observed": False, "status": INCONCLUSIVE,
                "reason": "Numeric per-trade returns and provenance are required."}
    wins = [x for x in returns if x > 0]; losses = [x for x in returns if x < 0]
    gross_profit = sum(wins); gross_loss = abs(sum(losses)); max_streak, streak_hist = _losing_streaks(returns)
    metrics: Dict[str, Any] = {
        ...
        "expectancy": mean(returns), "profit_factor": gross_profit / gross_loss if gross_loss > 0 else NOT_TESTED,
        "maximum_drawdown": _max_drawdown(returns), "maximum_losing_streak": max_streak,
        "losing_streak_distribution": streak_hist,
    }
    ...
    mc: Any = NOT_TESTED; iterations = receipt.get("monte_carlo_iterations"); seed = receipt.get("random_seed")
    if isinstance(iterations, int) and iterations > 0 and isinstance(seed, int):
        ...
        dds.sort(); mc = {"method": "trade_order_permutation", "iterations": iterations, "random_seed": seed,
                          "median_max_drawdown": _percentile(dds, .5), "p95_max_drawdown": _percentile(dds, .95),
                          "worst_max_drawdown": max(dds)}

    parameter_stability = _scenario_summary(receipt.get("parameter_scenarios"))
    regime_stability = _scenario_summary(receipt.get("regime_returns")); temporal = _scenario_summary(receipt.get("temporal_returns"))
    ...
    dataset_role = str(receipt.get("dataset_role") or "").strip().lower(); out_of_sample: Any = NOT_TESTED
    if dataset_role in {"untouched_test", "out_of_sample", "oos", "external_test"}:
        out_of_sample = {"state": RESULT_OBSERVED, "dataset_role": dataset_role, "sample_size": len(returns),
                         "expectancy": metrics["expectancy"], "profit_factor": metrics["profit_factor"],
                         "maximum_drawdown": metrics["maximum_drawdown"]}

    walk_forward: Any = NOT_TESTED; folds = receipt.get("walk_forward_folds")
    if isinstance(folds, Sequence) and not isinstance(folds, (str, bytes, bytearray)):
        fold_rows = []
        for i, raw in enumerate(folds, 1):
            values = _numbers(raw)
            if values: fold_rows.append({"fold": i, "n": len(values), "expectancy": mean(values), "maximum_drawdown": _max_drawdown(values)})
        if fold_rows:
    ...
    net_of_friction = receipt.get("returns_are_net_of_friction") is True
    decision = _decision(metrics, receipt.get("decision_rule")) if net_of_friction else {
        "status": INCONCLUSIVE,
        "reason": "returns_are_net_of_friction=true is required before a trading PASS/FAIL decision.",
    }
    return {"observed": True, "test_state": RESULT_OBSERVED, "status": decision["status"],
            "provenance": provenance, "unit": text(receipt.get("unit")), "metrics": metrics,
            "returns_are_net_of_friction": net_of_friction,
...
def find_result_receipt(result: Mapping[str, Any], hypothesis_id: str) -> Optional[Dict[str, Any]]:
    ...
    for key in ("validation_receipts", "experiment_results", "test_results"):
        raw = result.get(key)
        if isinstance(raw, Sequence) and not isinstance(raw, (str, bytes, bytearray)): candidates.extend(raw)
    ...
    for item in candidates:
        if isinstance(item, Mapping) and str(item.get("hypothesis_id") or item.get("id") or "") == str(hypothesis_id): return dict(item)
    return None
